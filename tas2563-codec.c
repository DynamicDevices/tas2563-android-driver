...
static int tas2563_set_slot(struct tas2563_priv *pTAS2563, int slot_width)
{
	...
	default:
		dev_err(pTAS2563->dev, "slot width not supported");
		ret = -EINVAL;
	}
...
static int tas2563_set_bitwidth(struct tas2563_priv *pTAS2563, int bitwidth)
{
	...
	default:
		dev_err(pTAS2563->dev, "Not supported params format\n");
	}
...
static int tas2563_set_samplerate(struct tas2563_priv *pTAS2563, int samplerate)
{
	...
	case 17640:
			...
			break;
	default:
			dev_err(pTAS2563->dev, "%s, unsupported sample rate, %d\n", __func__, samplerate);
		...
		struct snd_soc_dai *dai)
{
	...
	if(nResult < 0)
	{
		dev_err(pTAS2563->dev, "set bitwidth failed, %d\n", nResult);
		goto ret;
	}
